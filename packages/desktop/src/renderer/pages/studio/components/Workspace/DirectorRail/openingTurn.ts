...
export const DIRECTOR_PRESET_RULES = [
  ...
  '',
  'For approved references, follow this workflow in order:',
  ...
  'human-only and renderer-only; you cannot call it.',
  '5. Only after every character reference is approved, request background reference images.',
  '6. Wait for the person to approve background candidates in the References view.',
  '7. Call read_storyboard again, then use set_shot_reference_binding through studio_apply_edits once for',
  'each active Shot, with the exact approved reference IDs selected for that Shot. Record an explicit empty',
