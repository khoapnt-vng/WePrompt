...
describe('the schema-5 References workspace', () => {
  ...
  it('sizes the canonical picture and its takes from the project ratio, cropping neither', () => {
    ...
    expect(css).toMatch(
      /\.pictureBand\s*\{[^}]*aspect-ratio:\s*var\(--studio-reference-aspect-ratio\)[^}]*background:\s*var\(--studio-reference-matte\)/s
    );
    ...
    expect(css).toMatch(/\.take img\s*\{[^}]*aspect-ratio:\s*var\(--studio-reference-aspect-ratio\)/s);
