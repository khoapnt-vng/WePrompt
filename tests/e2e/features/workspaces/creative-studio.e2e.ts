...
const createRenderedCutFixture = async (page: Page, projectBrief: string): Promise<RenderedCutFixture> => {
  ...
  const prepared = await invokeStudioBridge<StudioRendererPreparedSubmissionOptionsV2>(page, 'prepare-submission', {
    ...
    baseChoices: [{ shotId, purpose: 'seed_still', referenceAssetId: null }],
    cascadeChoices: [{ shotId, purpose: 'video_take', referenceAssetId: null }],
  });
  ...
  test.describe.serial('Rendered Shot Gate-3 lifecycle', () => {
    test('trims and rerenders a continuous chain while proving stable lift refusal states', async ({
      ...
      const prepared = await invokeStudioBridge<StudioRendererPreparedSubmissionOptionsV2>(page, 'prepare-submission', {
        ...
        baseChoices: [{ shotId, purpose: 'seed_still', referenceAssetId: null }],
        cascadeChoices: [
          { shotId, purpose: 'video_take', referenceAssetId: null },
          { shotId: anchorShotId, purpose: 'video_take', referenceAssetId: null },
        ],
      });
      ...
      const rerenderPrepared = await invokeStudioBridge<StudioRendererPreparedSubmissionOptionsV2>(
        ...
        'prepare-submission',
        {
          ...
          originReferenceHandoffId: null,
          baseChoices: [{ shotId, purpose: 'video_take', referenceAssetId: null }],
          cascadeChoices: [{ shotId: anchorShotId, purpose: 'video_take', referenceAssetId: null }],
        }
      );
      ...
      await expect
        ...
        .toEqual([{ shotId, videoAssetId: replacementVideoAssetId, endpointSeconds: 8 }]);
