...
> Its own changelog reads: _"This supersedes the delivered table. Build from this one."_
>
...
>
> **Binding implementation normalization:** read those three residual `RUNNING` references
> as provider `GENERATING`. Region 4 is present for `GENERATING`, `PROPOSED`, and
> `NEEDS BUDGET`, and absent for local `RENDERING`. The designer bundles and their extracted
> renderings remain unchanged as delivery evidence.
