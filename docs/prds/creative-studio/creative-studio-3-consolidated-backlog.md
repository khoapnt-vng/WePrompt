...
**123 bugs filed · 121 closed · 2 open** — both P1 and both blocked on an owner decision rather than on engineering: **BUG-144** (ffmpeg distribution) and **BUG-165** (whether a chained take's quoted upper bound may double to fund a retry). _Separately, `TASKS.md` **BUG-028** was re-verified on 2026-08-29: the entry described deleted code, but the defect is real and was reproduced — a pending proposal dies permanently the moment any generation job ticks. Its fix is a proposal-schema and provenance change, not a bugfix, and awaits an owner decision._
