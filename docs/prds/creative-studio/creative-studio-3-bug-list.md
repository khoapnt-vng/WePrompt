...
- [ ] **[BUG-113][P2][Creative Studio] A transient provider failure leaves the Shot with no way forward** — measured 2026-08-23
  - Shot 3 of the bicycle film failed with `provider_unavailable` — a 5xx, unbilled, and by nature retryable. The Shot sits in `needs_attention` and the coverage bar says so, but **nothing offers a recovery**: the Beat panel shows no `Retry generation`, and every `Review video generation` control in the spend gate is disabled behind the same non-terminal job.
