...
- [ ] **[BUG-128][P1][Creative Studio] Three generation blockers render their raw i18n key, so the one message explaining why paid work is blocked is unreadable** — found 2026-08-26 by the owner on `6dadd0fc3`, in the `Sunday Kitchen` real-model run
...
- [ ] **[BUG-129][P2][Creative Studio] The Director asks the user to approve references that the product has already approved, and the request can never be satisfied** — found 2026-08-26 by the owner on `6dadd0fc3`
...
- [ ] **[BUG-130][P2][Creative Studio] The renderer blocks generation on a stale engine list while the backend reports every route ready** — found 2026-08-26 by the owner on `6dadd0fc3`
...
- [ ] **[BUG-131][P2][Platform] An invalid IPC payload makes `bridge.invoke` hang forever instead of rejecting** — found 2026-08-26 while driving Creative Studio through the renderer bridge
...
- [ ] **[BUG-133][P1][Creative Studio] Rendering a chain head on its own permanently strands its whole chain, and the only recovery is to pay for the head again** — found 2026-08-26 by the owner across two independent five-minute films
...
- [ ] **[BUG-134][P3][Creative Studio] The reference-capacity error counts characters and background together while the UI presents them as separate groups, so the number it reports does not match what the user can see** — found 2026-08-26 by the owner
