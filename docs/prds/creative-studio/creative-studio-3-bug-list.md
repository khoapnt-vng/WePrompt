...
- [ ] **[BUG-105][P2][Creative Studio] A continuity-frame failure is swallowed at both call sites, so the chain stalls with nothing in any log** — filed 2026-08-22 out of BUG-104
  - Two identical bare catches discard the extraction error: `jobManager.ts:1646` (_"The durable failed extraction remains visible to the explicit provider-free retry action"_) and `v2Service.ts:1961` (_"The durable failed extraction and waiting jobs are the recoverable result"_). Both comments are true — the failure **is** durable and retryable — but neither writes a line.
  - Underneath, `runLocalDecode` spawns ffmpeg with `-loglevel error` and stderr `ignore`, so the decoder's own diagnosis is discarded before it reaches the catch. For BUG-104 that discarded text was `stream 0, offset 0x54dc: partial file` — the whole answer, thrown away.
  - The result: a chain that stops dead with **zero** `[CreativeStudio]` lines. BUG-104 was diagnosed by reading `project.json` off disk and re-running ffmpeg by hand. A log line at the catch would have named the failing extraction and its error code in seconds.
  - Fix is small: `console.warn(formatStudioJobLog('conditioning_frame_failed', { projectId, extractionId, code }))` at both sites, matching BUG-101's shape. Capturing ffmpeg's stderr into the error is the larger, more valuable half and should be judged separately.
