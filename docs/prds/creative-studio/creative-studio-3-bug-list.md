...
- [x] **[BUG-112][P1][Creative Studio] Every video submission failed because the app's key bills a different, exhausted OpenRouter account** — root cause measured 2026-08-23
  - **The measured cause.** `GET /api/v1/credits` with the key the app uses returns `total_credits: 20, total_usage: 20.176` — the account this key bills has **$20 of credits, overdrawn by $0.18**. The $976.32 dashboard the user checked is a **different account**: its "Weprompt Andy" key carries a $50 limit and never expires, while the app's key reports `limit: null` and expires 2026-09-04. Two different keys, two different accounts; only one is funded, and the app spends from the other.
  - **The gate-order control removed the last doubt about the payload.** An empty `{}` POST returned **400 ZodError in 0.27s** (validation answers first), and the app's exact payload returned **402 in 0.96s** — so the payload is valid and the credit gate is the only live blocker.
  - **The timeline now closes exactly.** The $0.03 seed still succeeded at 23:01:58 UTC as the account crossed ~$20; six minutes later the first video — whose upfront estimate exceeded the pennies remaining — failed, and every video after it. The 400→402 shift near midnight UTC is provider-side presentation of the same exhausted state, not a second cause.
  - **Three hypotheses were refuted by experiment on the way** — seed encoding (94KB JPEG failed identically to 1.4MB PNG), prompt length (600 chars failed identically to 1028), and the key's own $50 cap (that cap belongs to a different key on the funded account). Each refutation was cheap and each prevented a wrong fix; the seed-encoding one would have put a transcode into a byte-integrity-checked path.
  - **What this hardened in the product**, all shipped: 402 → `quota` and other 4xx → `invalid_request` instead of `submission_unknown` (no more duplicate-charge acknowledgements for requests that never ran); provider error tags admitted by identifier shape rather than a fixed allowlist; `limit_source` extracted into the evidence, so the next spend-gate rejection names itself in the log.
  - **Remedy:** replace the app's OpenRouter key (Settings → Model → provider) with one created on the funded account — which also completes the rotation already recommended, since the current key is readable in plaintext via `/api/providers` and now sits in shell history. Alternatively, top up the $20 account.
