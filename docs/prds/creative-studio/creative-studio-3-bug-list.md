...
- [ ] **[BUG-112][P1][Creative Studio] Every video submission is rejected for a spend reason the account does not appear to have** — 2026-08-23, cause still open
  - Replaying the exact submission outside the app, against the key the app uses, returns HTTP **402 · `Insufficient credits`** with `limit_source: openrouter_credits`. That is the measured fact.
  - **What that does not yet explain.** The account balance is **$976.32**, the key's credit limit is **$50**, and the key's reported spend over thirty days is **$10.92**. None of those numbers describes an exhausted allowance, so "the key is capped out" is an inference the evidence does not support. It was written into this entry once and is withdrawn.
  - Open lines: whether the $50 cap counts lifetime rather than thirty-day spend; whether video generation is priced or authorised differently from the chat models in that graph; and why the app receives **400** where the replay receives **402** for the same key and payload. `GET /api/v1/key` reports the key's own `limit`, `usage` and `limit_remaining` and should settle the first two.
  - **Two hypotheses already refuted by experiment**, both cheaply, and both worth the cost:
    1. _Seed encoding_ — the film that rendered had a 616KB JPEG seed, the failing one a 1.4MB PNG. Re-encoding the identical image to 94,585 bytes and re-rendering failed identically. The fix that hypothesis implied was a transcode inside `resolveProviderInputV2`, a byte-integrity-checked path; confirming first is what kept that risk out of the codebase.
    2. _Prompt length_ — 1028 characters on the failing film against 637–641 on the one that rendered, with the Beat panel already showing `55 / 25 WORDS`. A 600-character replay returned the same status.
  - **The product's own handling is a separate defect and is fixed.** `mapStatusError` let every status outside 401/403/429/5xx fall through to `unknown`, which becomes `submission_unknown` — parking the Shot behind an acknowledgement about paying twice, five times over, for work that was never charged. 402 now maps to `quota` and other 4xx to `invalid_request`.
  - **The fact that would have shortened all of this was in hand throughout:** every 400 logged `messagePresent: true`, the flag saying the provider had sent a reason, without the reason. See BUG-111.
