...
- [x] **[BUG-112][P1][Creative Studio] Five video submissions failed against a capped API key, and the product called it an unknown outcome** — 2026-08-23, root cause found
  - **Cause: the OpenRouter key carries a $50 per-key credit limit with no reset, and it had consumed it.** The account balance was $976.32 throughout, which is why the balance was never the constraint and why the dashboard looked healthy. Confirmed by replaying the exact submission outside the app: HTTP **402**, `Insufficient credits`, `limit_source: openrouter_credits`, on a key whose last characters match the one the app uses.
  - **Two hypotheses were refuted by experiment before this one, and both refutations were worth the cost.**
    1. _Seed encoding._ The film that rendered had a 616KB JPEG seed; the film that failed had a 1.4MB PNG. Re-encoding the identical image to a 94,585-byte JPEG and re-rendering produced the same failure. The fix that hypothesis implied was a transcode inside `resolveProviderInputV2`, a byte-integrity-checked path — built on a guess, it would have added risk for nothing.
    2. _Prompt length._ The composed prompt is 1028 characters on the failing film against 637–641 on the one that rendered, and the Beat panel shows `55 / 25 WORDS` on that Beat's Look. Replaying at 600 characters returned the same 402.
  - **What the product did with it.** `mapStatusError` handled 401/403, 429 and 5xx and let everything else fall through to `unknown`, which becomes `submission_unknown`: the Shot parks in `needs_attention` behind an acknowledgement whose purpose is accepting the risk of paying twice. For a spend limit that charged nothing, five times over. Fixed — 402 now maps to `quota`, other 4xx to `invalid_request`, and both codes were added to the adapters' sanitized union.
  - **The one fact that would have ended this in a minute was in hand the whole time.** Every one of those 400s logged `messagePresent: true` — the flag saying the provider sent a human-readable reason, without the reason. See BUG-111; that redaction is correct and the message very likely said `Insufficient credits`.
  - Remedy for the run itself: raise or clear the key's credit limit at `openrouter.ai/settings/keys`.
