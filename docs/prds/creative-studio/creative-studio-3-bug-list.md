...
- [ ] **[BUG-143][P1][Creative Studio] Cancelling a film export during `analyzing` refuses the cancellation and then blames the user's media** — found 2026-08-27 driving the newly landed film export (`13ac6ee99`) on `Light on Water`
...
- [ ] **[BUG-146][P3][Creative Studio] Film export copy carries a factual error and collides with the name of a destructive operation** — found 2026-08-27 reviewing all 34 `filmExport.*` strings against the app's own vocabulary
...
- [ ] **[BUG-149][P2][Creative Studio][UNVERIFIED — NOT REPRODUCED] A dissolve may cross a Beat boundary where the Cut should hard-cut** — raised 2026-08-27 from a code reading during film-export verification; **no live reproduction exists and the claim may be wrong**
  - **Status, stated plainly.** This entry records a code-reading hypothesis that the live run **could not test**. It is not a confirmed defect. Do not act on it without reproducing it first.
  - **The hypothesis.** `filmExporter.ts:1121-1131` gates a dissolve on `chainBreak === 'none'` alone, but this codebase's chain-head rule is `shotIndex === 0` **or** `hard_cut` (`schema2/chain.ts:204`), and every Shot is minted `chainBreak: 'none'` (`mutations/index.ts:1595`). `timelineEntries` flattens Beats (`filmExporter.ts:1538`). If all three hold, the first Shot of Beat 2 would cross-fade into the last Shot of Beat 1 instead of cutting, and the mirrored validator at `catalog.ts:690` would ratify it.
  - **Why it could not be tested.** `Light on Water` is **1 Beat / 3 Shots**, so both joins are intra-Beat. The dissolve export produced exactly 2 dissolves (28.375s of segments − 2 × 0.3333s = 27.708s rendered), which is the correct result **whether or not the bug exists**. The test is not discriminating on a single-Beat project.
  - **How to settle it.** On a project with **≥2 Beats**, count the expected eligible joins as `sum(shots per Beat − 1)` and compare against the dissolve facts in `artifact.json`. If `renderedDurationSeconds` reflects `dissolveSeconds × (totalShots − 1)` rather than `× intra-Beat joins`, the bug is live. Confirm by eye at the Beat boundary — it will cross-fade where the Cut shows a cut.
  - **If confirmed**, the fix is to gate the dissolve on the same chain-head predicate the rest of the engine uses, rather than on `chainBreak` alone, and to make the validator share that predicate instead of mirroring the exporter's version of it.
