...
- [ ] **[BUG-104][P1][Creative Studio] A chain cannot advance past its first Shot: the continuity frame never releases the next take** — found 2026-08-22 in the end-to-end run that finally produced a video
  - The run got further than any before it. One Beat, three Shots, cascade authorized at **$0.63** in a single gate. The seed still succeeded, and **`shot_dawn_light` produced a real 1.3MB MP4** — the second video this product has ever made.
  - Then it stops. `shot_mist_lifts` and `shot_single_ripple` have sat at `waiting_for_conditioning` with the project revision frozen at 28 for over seven minutes and **nothing in flight**.
  - The sequence: after selecting the upstream video take, the panel reported **"Continuity-frame preparation failed."** with a free `Retry continuity frame` action. The retry ran — `frameExtractions` went to 1 and the failure text cleared — but the panel now reads **"Preparing the continuity frame."** indefinitely, and the waiting take never resumes.
  - Codex's five-second run loop (`runtime.ts`, landed today) calls `resumePendingJobsV2` across supported projects and does not pick this up, so whatever gates the deferred take is not the same thing the loop resumes.
  - The only remaining affordance is **Cancel waiting item**. There is no way forward, only a way to abandon two already-authorized generations.
  - This is the real ceiling on the product today: a single-Shot Beat can be rendered, and a multi-Shot Beat cannot. Every film with continuity stops after its first Shot.
  - **BUG-101's logging earned itself here.** Both submissions are on the record — `[CreativeStudio] submit … purpose=seed_still kind=image` and `… purpose=video_take kind=video model=bytedance…` — which is how the successful half was confirmed without reading JSON off disk. The stall itself writes nothing, because frame extraction is not instrumented.
