...
- **D1** puts sound direction in `shootingScript` — free text the model writes — explicitly
  *instead of* a new field, to avoid an unmigrated schema change.
...
So CS3 settled how a person *asks* for sound, not what a produced sound *is*. Phase 6 needs
...
the first attribute we have that one kind owns *about* another.
