...
> **STATUS — SIGNED IN FULL, 2026-09-02.**
>
> Binding in full: `PARTIAL` counts missing picture only; the status contract adds
> `QUEUED`, `READY TO RENDER` and `SLATE`; and provider `GENERATING` remains distinct from
> local `RENDERING`. The owner also explicitly confirmed these four clauses:
>
