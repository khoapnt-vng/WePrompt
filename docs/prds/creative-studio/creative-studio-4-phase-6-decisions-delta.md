...
Your rule: *"Missing/failed members block export."* As written this blocks a film that has no
...
> cut's audio lane — *'narration for Beat 2 — not generated'* — never as a block-level status.
...
- **`READY TO RENDER`** — member level, and it appears *only* where a member could start and a
