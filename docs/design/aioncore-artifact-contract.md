...
| Migration lineage               | Archive root: `migration-lineage.json`, UTF-8 JSON. Its format is the repository's accepted document: `schemaVersion`, `minimumSupportedVersion`, `latestVersion`, `entryCount`, `fingerprint`, and ordered `entries` of `version`, `description`, and `checksum` (`packages/shared-scripts/src/aioncore-migration-lineage.json:1-11`).                                                                         | All three remote resolvers require the file inside the extracted archive and deep-compare it with WePrompt's accepted lineage (`prepare-aioncore.js:148-161`, `747-753`, `787-795`, `868-875`). The final verifier repeats the deep comparison (`verify-bundled-aioncore-resources.js:159-195`).                                                                                                                                                                                                                                          | **Verified current requirement. Blocking gap:** the repository records that the publishing workflow archives only the binary, so no current release-line archive contains this required file (`TASKS.md:52-57`; `aioncore-build-provenance-handoff.md:20-25`).                                                                                                      |
