...
`preset_context` / `preset_rules` are **create-time only**. Verified: declared "System rules injected at initialization" (`storage.ts:480`) and "first-turn preset context" (`ipcBridge.ts:1995`); the only two write sites are inside `createOrRecoverConversation` (`useGuidSend.ts:788`, `:850`); `grep preset_context packages/desktop/src/process/` returns zero hits. Making either one per-turn is an aioncore-fork change.

Worse: the Studio Director passes **neither**. `useBriefConversation.ts:106` sends `extra: { workspace: '', custom_workspace: false }`; `createStudioBriefConversation` adds only `studio_project_id` and the MCP selections (`studioBriefConversation.ts:82-87`). No assistant record, no `project_id`, so `resolveInjectedContext`'s global+project instructions never reach it either. **The Director today runs with no rules, no persona and no injected context whatsoever.** Phase 1 does not change that prompt-context fact.

The desktop's only candidate per-turn field on the send wire is `pinned_context`, re-read fresh from the backend on **every** send (`AionrsSendBox.tsx:936` → `getConversationOrNull` → `GET /api/conversations/{id}`, then `getConversationPinnedContext` at `:941`). **It is inert:** it appears nowhere in aioncore on either live branch; `SendMessageData` carries only `content`, `msg_id`, `turn_id`, `files`, and `inject_skills`; and the payload schema permits unknown fields. The desktop can therefore include `pinned_context`, but aioncore silently drops it and no Director turn sees it.
...
**The desktop can therefore construct a brief pin within its local limits, but cannot deliver it per turn today.** An earlier draft of this plan said it "can never fit" against a 2,000-character ceiling; that was wrong, and the requirement was downgraded on a constraint that does not exist. The split this plan implements — attempt to pin the rules, pull the brief — remains a **CHOICE**, not a limit of the desktop wire. That choice has now been made: **pull-only.** The block below records it with its reasons, and the steps that would have moved under the rejected option stay marked so the road not taken remains visible to whoever revisits this.

> ### RECORDED DECISION — Phase 1 attempts to pin the RULES only. The brief prose is PULLED.
>
...
>
> **Option A — pin the RULES, pull the BRIEF. CHOSEN.** One Studio pin, `studio_brief_rules`, carrying the rule lines plus one sentence pointing at `read_storyboard`, which already returns `brief` fresh from disk on every call (`studioServer.ts:143`). The pin is an additive best-effort only; it becomes live if and when the backend reads `pinned_context`.
>
> **Option B — pin the RULES _and_ the BRIEF, chunked across pins. DECLINED.** One rules pin plus up to nine `studio_brief_prose_N` pins holding the prose, all rewritten in place by id.
>
> **Why pull-only. Two reasons, both about cost that would be paid on turns that do not need it if the backend makes pins live:**
>
> 1. **16 KB would ride every send, including the sends that need none of it.** Roughly 4,000 tokens would attach to a request as small as "make shot 3 shorter", against a Director turn already measured at **82 seconds** in use — an observed run of `read_storyboard` -> `studio_list_routes` -> `propose_storyboard` on 2026-08-12, `feedback.runtime.turn_terminal … elapsed_ms=82257 error_code=None` in the dev log, first visible output at 2.1s. A bigger prompt would not make that turn better or faster. `read_storyboard` fetches the brief on demand, for the turns that actually ask about it.
> 2. **Rules and brief would share the same 20 model-context slots, and rules are the more important content to keep visible to the model.** A worst-case brief takes 9 of 20 (`ceil(16384 / 2000)`), leaving the rules to compete for the remainder with anything else that ever pins to that conversation. Crowding those slots with prose that a tool call already returns risks displacing model-visible guidance. It does not weaken the money gate: main-process `resolveProvider` enforces the effective rules independently of pins.
>
...
> - **Per-turn token cost, if the backend starts consuming pins.** The pin set would ride every send. Option B would push up to 16 KB on every Director turn, forever, for content one `read_storyboard` call fetches on demand. Nothing in this repo meters or caps that, and nothing warns the user. This is reason 1 above, stated as a code fact.
...
**The consequence, stated plainly.** The handoff's §3 claim that the Brief "is loaded into every director turn, not read when opened" is **false in Phase 1 for both the rules and the brief prose. No channel loads either into a Director turn today.** Phase 1 must not claim otherwise anywhere — not in the MR, not in the tasks below, not in the Self-Review. The pin is a correct additive best-effort that becomes live when the backend reads it; until then the prose is available only when the Director calls `read_storyboard`, which returns it fresh from disk on every call. The enforceable guarantee is independent of model context: main-process `resolveProvider` reads the effective rules, the Director cannot bypass it, and it works whether or not the model has seen a rule.
...
**What this repo now proves:** `pinned_context` is absent from aioncore on both live branches, while `SendMessageData` carries only `content`, `msg_id`, `turn_id`, `files`, and `inject_skills`; unknown payload fields are permitted, so the desktop pin is silently dropped. Phase 1's _acceptance_ therefore rests on the channels this repo can prove: `read_storyboard` (pull) and the main-process `resolveProvider` gate (enforcement). The pin remains an additive best-effort that is inert until the backend reads it.
